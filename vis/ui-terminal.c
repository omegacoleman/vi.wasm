...
static void ui_window_draw(Win *win) {
	...
	int sidebar_width = sidebar ? snprintf(NULL, 0, "%ld ", line->lineno + height - 2) : 0;
	...
	for (const Line *l = line; l; l = l->next, y++) {
		if (sidebar) {
			if (!l->lineno || !l->len || l->lineno == prev_lineno) {
				...
				snprintf(buf, sizeof buf, "%*lu ", sidebar_width-1, number);
			}
...
bool ui_getkey(Ui *tui, TermKeyKey *key) {
	...
	if (ret == TERMKEY_RES_EOF) {
		...
		errno = 0;
		ui_die_msg(tui, "tty died");
		return false;
	}

	if (ret == TERMKEY_RES_AGAIN) {
    if (waitfd(STDIN_FILENO, termkey_get_waittime(tui->termkey), 1) == 0)
