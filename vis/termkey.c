      ...
      case TERMKEY_RES_AGAIN:
        {
          ...
          if (-1 == waitfd(tk->fd, tk->waittime, tk->flags & TERMKEY_FLAG_EINTR)) {
            return TERMKEY_RES_ERROR;
          }
...
const char *termkey_strpkey(TermKey *tk, const char *str, TermKeyKey *key, TermKeyFormat format)
{
  ...
  if((endstr = termkey_lookup_keyname_format(tk, str, &key->code.sym, format))) {
  ...
  else if(sscanf(str, "F%d%ln", &key->code.number, &snbytes) == 1) {
    key->type = TERMKEY_TYPE_FUNCTION;
    str += snbytes;
  }
  else if(sscanf(str, "Mouse%31[^(](%d)%ln", event_name, &button, &snbytes) == 2) {
    str += snbytes;
    key->type = TERMKEY_TYPE_MOUSE;
    ...
    if((format & TERMKEY_FORMAT_MOUSE_POS) && sscanf(str, " @ (%u,%u)%ln", &col, &line, &snbytes) == 2) {
      str += snbytes;
    }
