...
size_t text_restore(Text *txt, time_t time) {
	...
	while (time > rev->time && rev->later)
		rev = rev->later;
	time_t diff = llabs(rev->time - time);
	if (rev->earlier && rev->earlier != txt->history && llabs(rev->earlier->time - time) < diff)
		rev = rev->earlier;
	if (rev->later && rev->later != txt->history && llabs(rev->later->time - time) < diff)
		rev = rev->later;
...
Text *text_load_method(const char *filename, enum TextLoadMethod method) {
	...
	if (filename) {
		errno = 0;
		block = block_load(filename, method, &txt->info);
		if (!block && errno)
			goto out;
